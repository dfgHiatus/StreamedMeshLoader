...
/// <summary>
/// A basic Udon Coroutine, which executes an inner list of Udon Coroutines in parallel, finishing when all inner coroutines are complete.
/// </summary>
public sealed class WaitForAllCoroutine : BaseUdonCoroutine
{
    ...
    protected override bool Tick()
    {
        for (int i = 0; i < coroutines.Length; i++)
        {
            if (!coroutines[i].Completed) return false;
        }
